...
public class Parser {
    ...
    boolean startingDecl() {
        return (isNextTok( Tokens.Int ) || isNextTok( Tokens.BOOLean) || isNextTok(Tokens.STring) || isNextTok(Tokens.Char) ) ;
    }
    ...
    public AST rType() throws SyntaxError {
        ...
        if( isNextTok( Tokens.Int )) {
            ...
            type = new CharTypeTree();
            scan();
            
        }else if(isNextTok(Tokens.STring)){
            type = new StringTypeTree();
            scan();
    ...
    public AST rFactor() throws SyntaxError {
        ...
        if (isNextTok(Tokens.CHARacter)) {
            expression = new CharTree( currentToken );
            scan();
            return expression;
        }
        ...
        if (isNextTok(Tokens.StringLit)) {
            expression = new StringTree( currentToken );
            scan();
            return expression;
        }
